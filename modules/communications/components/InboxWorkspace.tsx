          ...
          <div className="grid gap-4 xl:grid-cols-[340px_minmax(0,1fr)]">
            <div className="space-y-2 border-r pr-2">
              ...
              {readModel.conversations.map((item) => (
                <button
                  className={`ui-list-item ui-list-item-hover w-full text-left ${selectedConversation?.conversation.id === item.conversation.id ? "border-link/45" : ""}`}
                ...
                >
                  ...
                  {item.pendingSuggestionCount > 0 ? (
                    <p className="mt-1 text-xs font-semibold text-link">{item.pendingSuggestionCount} suggested matches</p>
                  ) : null}
                </button>
              ))}
            </div>
