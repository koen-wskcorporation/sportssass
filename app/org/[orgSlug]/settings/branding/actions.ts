...
export async function saveOrgBrandingAction(orgSlug: string, formData: FormData) {
  ...
  if (primaryColor && !isValidHexColor(primaryColor)) {
    redirect(`/org/${orgSlug}/settings/branding?error=invalid_primary`);
  }

  if (secondaryColor && !isValidHexColor(secondaryColor)) {
    redirect(`/org/${orgSlug}/settings/branding?error=invalid_secondary`);
  }
  ...
  try {
    ...
    redirect(`/org/${orgSlug}/settings/branding?error=upload_failed`);
  }
  ...
  if (error) {
    redirect(`/org/${orgSlug}/settings/branding?error=save_failed`);
  }

  redirect(`/org/${orgSlug}/settings/branding?saved=1`);
}
