...
import { StructureCanvasShell, type StructureSearchItem } from "@/src/features/core/structure/components/StructureCanvasShell";
import { StructureNode } from "@/src/features/core/structure/components/StructureNode";
import { getFacilityPolygonGeometry } from "@/src/features/facilities/utils/polygon-geometry";
...
function getRoomLayout(space: FacilitySpace, index: number) {
  ...
  const snappedLeft = snapToGrid(geometry.bounds.left);
  const snappedTop = snapToGrid(geometry.bounds.top);
  const snappedWidth = snapSizeToGrid(geometry.bounds.width);
  const snappedHeight = snapSizeToGrid(geometry.bounds.height);
  const polygonPoints = geometry.points.map((point) => ({
    x: (point.x - geometry.bounds.left) / Math.max(1, geometry.bounds.width),
    y: (point.y - geometry.bounds.top) / Math.max(1, geometry.bounds.height)
  }));
...
export function StructureCanvas(props: StructureCanvasProps) {
  ...
  const resolvedOnFit =
    ...
    ((options?: { viewportOffsetX?: number; viewportOffsetY?: number; animated?: boolean }) => {
      canvasRef.current?.fitToContentSelector("[data-structure-node-id]", options);
    });
