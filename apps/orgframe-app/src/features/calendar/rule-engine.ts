import type { CalendarOccurrence, CalendarRule } from "@/src/features/calendar/types";

export const DEFAULT_CALENDAR_HORIZON_MONTHS = 18;

export type GeneratedCalendarOccurrenceInput = Pick<
  CalendarOccurrence,
  | "entryId"
  | "sourceRuleId"
  | "sourceType"
...
function normalizeTimeBounds(rule: CalendarRule) {
...
function buildSourceKey(rule: CalendarRule, localDate: string, localStartTime: string | null) {
  return `calendar-rule:${rule.id}:${localDate}:${localStartTime ?? "all-day"}:${rule.timezone}`;
}

function computeCandidateDates(rule: CalendarRule, nowDate: Date, horizonMonths: number): string[] {
  const { mode } = rule;
  ...
  const windowEnd = [untilFromMode, explicitEnd, horizonEnd]
    .filter((value): value is Date => Boolean(value))
    .sort((a, b) => a.getTime() - b.getTime())[0] ?? horizonEnd;
...
export function generateOccurrencesForRule(
  rule: CalendarRule,
  options?: { nowDate?: Date; horizonMonths?: number }
): GeneratedCalendarOccurrenceInput[] {
  if (!rule.isActive) {
    return [];
  }

  const nowDate = options?.nowDate ?? new Date();
  const horizonMonths = options?.horizonMonths ?? DEFAULT_CALENDAR_HORIZON_MONTHS;
  ...
  return selectedDates.map((localDate) => {
    ...
    } satisfies GeneratedCalendarOccurrenceInput;
  });
}
