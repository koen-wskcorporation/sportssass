...
import {
  ...
  translatePolygon,
  type PolygonPoint
} from "@/src/features/facilities/utils/polygon-geometry";
                ...
                editContent={
                  <>
                    {rooms.map((room) => {
                      ...
                      const normalizedPolygonPoints = localPolygonPoints.map((point) => ({
                        x: point.x / Math.max(1, layout.width),
                        y: point.y / Math.max(1, layout.height)
                      }));
                      const localCentroid = getPolygonCentroid(localPolygonPoints);
                      // Use the exact editable control polygon for rendered shape edges so
                      // the visible fill/outline always matches selectable vertices.
                      const roundedPath = buildRoundedPolygonPath(localPolygonPoints, 0);
                      ...
                      return (
                        <StructureNode
                          capabilityMode={mapCanEdit ? "editable" : "static"}
                          chromeless
                          centerContent
                          chipsAboveTitle
                          ...
                          className={`group pointer-events-auto absolute ${isDraggingThisRoom ? "transition-none" : ""}`}
                          chips={null}
                          conflicted={false}
                          focused={false}
                          ...
                          nodeId={room.id}
                          key={room.id}
                          polygonPoints={normalizedPolygonPoints}
                          shape="polygon"
                          style={{
                            left: `${layout.x}px`,
                            top: `${layout.y}px`,
