import type { FacilityReservation, FacilityReservationRule } from "@/src/features/facilities/types";

export const DEFAULT_FACILITY_SCHEDULE_HORIZON_MONTHS = 18;

export type GeneratedFacilityReservationInput = Pick<
  FacilityReservation,
  | "sourceKey"
  | "spaceId"
  | "sourceRuleId"
...
function normalizeTimeBounds(rule: FacilityReservationRule) {
...
function buildSourceKey(rule: FacilityReservationRule, localDate: string, localStartTime: string | null) {
  return `facility-rule:${rule.id}:${localDate}:${localStartTime ?? "all-day"}:${rule.timezone}`;
}

function computeCandidateDates(rule: FacilityReservationRule, nowDate: Date, horizonMonths: number): string[] {
  const { mode } = rule;
...
export function generateReservationsForRule(rule: FacilityReservationRule, options?: { nowDate?: Date; horizonMonths?: number }): GeneratedFacilityReservationInput[] {
  if (!rule.isActive) {
    return [];
  }

  const nowDate = options?.nowDate ?? new Date();
  const horizonMonths = options?.horizonMonths ?? DEFAULT_FACILITY_SCHEDULE_HORIZON_MONTHS;
  ...
  return selectedDates.map((localDate) => {
    ...
    return {
      ...
      reservationKind: rule.reservationKind,
      ...
      conflictOverride: rule.conflictOverride,
      metadataJson: {
        mode: rule.mode,
        generatedAt: new Date().toISOString()
      }
    } satisfies GeneratedFacilityReservationInput;
  });
}
