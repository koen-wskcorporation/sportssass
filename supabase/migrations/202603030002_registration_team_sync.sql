...
begin
  ...
  if resolved_form_kind = 'program_registration' then
    ...
    loop
      ...
      entry_node_kind := null;

      if entry_node_id is not null then
        select node.capacity, node.waitlist_enabled, node.node_kind
        into node_capacity, node_waitlist_enabled, entry_node_kind
      ...
      returning id into inserted_registration_id;

      if entry_node_kind = 'team' then
        select team.id
        into resolved_team_id
        from public.program_teams team
        where team.program_node_id = entry_node_id
        limit 1;

        if resolved_team_id is null then
          insert into public.program_teams (org_id, program_id, program_node_id)
          values (resolved_org_id, resolved_program_id, entry_node_id)
          on conflict (program_node_id) do update set program_node_id = excluded.program_node_id
          returning id into resolved_team_id;
        end if;

        if entry_status in ('submitted', 'in_review') then
          team_member_status := 'pending';
        elsif entry_status = 'approved' then
          team_member_status := 'active';
        elsif entry_status = 'waitlisted' then
          team_member_status := 'waitlisted';
        else
          team_member_status := 'removed';
        end if;

        if resolved_team_id is not null then
          insert into public.program_team_members (
            team_id,
            org_id,
            program_id,
            player_id,
          ...
          values (
            ...
            resolved_program_id,
            entry_player_id,
            inserted_registration_id,
            team_member_status,
            'player',
            auth.uid()
          )
          on conflict (program_id, player_id) where status in ('active', 'pending', 'waitlisted')
          do update set
