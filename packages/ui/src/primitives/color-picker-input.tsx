...
export function ColorPickerInput({ name, defaultValue, value, onChange, placeholder = "00EAFF", disabled, className }: ColorPickerInputProps) {
  const pickerRef = React.useRef<HTMLInputElement | null>(null);
  const isControlled = value !== undefined;
  const [internalRawHex, setInternalRawHex] = React.useState(() => sanitizeHexInput((defaultValue ?? "").replace(/^#/, "")));
  const rawHex = isControlled ? sanitizeHexInput((value ?? "").replace(/^#/, "")) : internalRawHex;
  ...
  const setRawHex = React.useCallback(
    (nextRawHex: string) => {
      const sanitized = sanitizeHexInput(nextRawHex);

      if (!isControlled) {
        setInternalRawHex(sanitized);
      }

      onChange?.(sanitized.length === 0 ? "" : `#${sanitized}`);
    },
  ...
  return (
    <div className={cn("w-full", className)}>
      <div className="flex h-10 w-full items-center rounded-control border border-border bg-surface pr-2 text-sm shadow-[inset_0_1px_0_hsl(var(--canvas)/0.35)] focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 focus-within:ring-offset-canvas">
        <button
        ...
        <input
          ...
          disabled={disabled}
          onChange={(event) => {
            const sanitized = sanitizeHexInput(event.currentTarget.value);
      ...
      <input
        ...
        disabled={disabled}
        onChange={(event) => {
          setRawHex(event.currentTarget.value.replace(/^#/, ""));
        }}
        ref={pickerRef}
        type="color"
        value={pickerColor}
